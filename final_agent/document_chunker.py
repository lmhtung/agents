...
class DocumentChuncker:
    def __init__(self):
        self.__parent_splitter = MarkdownHeaderTextSplitter(
            headers_to_split_on=cfg["rag"]["headers_to_split_on"], 
            strip_headers=False
        )
        self.__child_splitter = RecursiveCharacterTextSplitter(
            chunk_size=cfg["rag"]["child_chunk_size"], 
            chunk_overlap=cfg["rag"]["child_chunk_overlap"]
        )
        self.__min_parent_size = cfg["rag"]["min_parent_size"]
        self.__max_parent_size = cfg["rag"]["max_parent_size"]

    def create_chunks(self, path_dir=cfg["directories"]["markdown_dir"]):
